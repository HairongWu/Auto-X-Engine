...
def main():
    ...
    if arch_config["algorithm"] in [
        ...
        for idx, name in enumerate(model.model_name_list):
            sub_model_save_path = os.path.join(save_path, name, "model")
            export_single_model(
    ...
    out_path = config["Global"]["save_inference_dir"]
    if os.path.exists(os.path.join(out_path,"Student")):
        out_path = os.path.join(out_path,"Student")
    config2 = CxxConfig()
    ...
    input_tensor.from_numpy(np.ones([1,3, 48, 320]).astype('float32'))
